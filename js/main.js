class CoreDomElements {
    ...
    showLoader() {
        ...
        if(containerFirstChild.classList.contains('lds-ellipsis')) throw new Error('Loader jest już aktywny, nie można dodac drugiego');
    ...
    hideError() {
        const errorMessageElement = document.querySelector('.search-view__form .search-view__error-message') || '';
...
class Weather {
    ...
    getApiURL(type) {
        switch(type) {
            ...
            case 'weekly':
                ...
                break;
            case 'hourly':
                return `https://weatherbit-v1-mashape.p.rapidapi.com/forecast/3hourly?city=${this.city}`;
                break;
        }
    }
    ...
    async fetchWeatherApi(URL, options) {
        try {
            ...
            if(response.statusText !== 'OK') {
                this.domElements.hideLoader();// po kolejnym zapytaniu api i odpowiedzi próbuje usunąc loader którego już nie ma i wyskakuje błąd.
                this.domElements.showSearch();
                if(response.statusText === 'No Content') throw new Error('Nie znaleziono takiej lokalizacji, spróbuj ponownie');
    ...
    renderWeekly(data) {
        this.domElements.dayForecastElements.forEach((day, index) => {
            day.querySelector('img').src = `https://www.weatherbit.io/static/img/icons/${data[index + 1].weather.icon}.png`;
            day.querySelector('.day-temperature').innerText = `${data[index + 1].high_temp.toFixed(0)}°`;
            day.querySelector('.night-temperature').innerText = `${data[index + 1].low_temp.toFixed(0)}°`;
